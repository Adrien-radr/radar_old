...
{
    if(evt.type == EMousePressed && evt.button < 3) {
        g_MousePressed[evt.button] = true;
    }
    else if(evt.type == EMouseWheelMoved) {
        g_MouseWheel += (float)evt.i;
    }
}

void ImGui_KeyListener(const Event &evt, void *data) {
    ImGuiIO& io = ImGui::GetIO();
    if(evt.type == EKeyPressed) {
        io.KeysDown[evt.i] = true;
    }
    else if(evt.type == EKeyReleased) {
        io.KeysDown[evt.i] = false;
    }
    else if(evt.type == ECharPressed && evt.i > 0 && evt.i < 0x10000) {
        io.AddInputCharacter((unsigned short) evt.i);
    }
...
bool Device::ImGui_CreateDeviceObjects() {
    // Backup GL state
...
void Device::ImGui_NewFrame() {
    ...
    w = windowSize.x; display_w = w;
    h = windowSize.y; display_h = h;
