 ...
 */
inline Eigen::VectorXf CosSumIntegralEigen(float x, float y, float c, int n) {
...
inline Eigen::VectorXf LineIntegralEigen(const Vector& A, const Vector& B,
   ...
   const auto a = w.Dot(s);
   const auto b = w.Dot(t);
   ...
   // a shifted cosine.
   const auto r = s.Dot(B) / B.Dot(B);
   const auto l = acos(clamp<float>(r, -1.0f, 1.0f));
   const auto p = atan2(b, a);

   return CosSumIntegralEigen(-p, l-p, c, n);
}
...
inline Eigen::VectorXf BoundaryIntegralEigen(const Polygon& P, const Vector& w,
   ...
   for(auto& edge : P.edges) {
      ...
      auto normal = edge.A.Cross(edge.B);
	  normal.Normalize();

      // Add the egde integral to the total integral
      const auto dotNV   = normal.Dot(v);
      const auto lineInt = LineIntegralEigen<Vector>(edge.A, edge.B, w, n);
      b += dotNV * lineInt;
   }
 ...
 */
template<class Polygon, class Vector>
inline float SolidAngleEigen(const Polygon& P) {
   if(P.size() == 3) {
 ...
 */
template<class Polygon, class Vector>
inline bool CheckPolygonEigen(const Polygon& P) {
   ...
   } else if(P.edges.size() == 3) {
      // Check with respect to centroid
      const auto D = (P.edges[0].A + P.edges[1].A + P.edges[2].A) / 3.0f;
      const auto N = Vector::Cross(P.edges[1].A-P.edges[0].A, P.edges[2].A-P.edges[0].A);
   ...
   // General computation
      ...
      // corner.
      unsigned int K = 0;
      const Vector* minX = &P.edges[0].B;
      for(unsigned int k=1; k<P.edges.size(); ++k) {
         const Vector* X = &P.edges[k].B;
      ...
      const int K2 = (K < P.edges.size()-1) ? K+1 : 0;
      const auto D = (P.edges[K].A + P.edges[K].B + P.edges[K2].B) / 3.0f;
      const auto N = Vector::Cross(P.edges[K].B-P.edges[K].A, P.edges[K2].B-P.edges[K].A);
 ...
 */
template<class Polygon, class Vector>
inline Eigen::VectorXf AxialMomentEigen(const Polygon& P, const Vector& w, int n) {

   // Check if the polygon is well oriented
   //const bool check = CheckPolygonEigen<Polygon, Vector>(P);
   //if(!check) {
      //return Eigen::VectorXf::Zero(n+2);
   //}

   // Arvo's boundary integral for single vector moment.
   Eigen::VectorXf a = - BoundaryIntegralEigen<Polygon, Vector>(P, w, w, n);
...
inline Eigen::VectorXf AxialMomentsEigen(const Polygon& P,
   ...
   for(int i=0; i<dsize; ++i) {
      ...
      const auto In = AxialMomentEigen<Polygon, Vector>(P, w, order-1);
