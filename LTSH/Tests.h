...
inline Eigen::MatrixXf ZonalWeightsEigen(int order) {
	Eigen::MatrixXf W = Eigen::MatrixXf::Zero(order, order);
	W(0, 0) = 1.0f;
	W(1, 1) = 1.0f;
	for (int n = 2; n<order; ++n) {
		const int subsize = n;
		W.row(n).segment(1, subsize) = (2.f * n - 1) * W.row(n - 1).segment(0, subsize);
		W.row(n) -= (n - 1.f) * W.row(n - 2);
		W.row(n) /= (f32) n;
	}

	// Scale each columns by the corresponding √(2l+1 / 4π) factor.
	const float factor = 1.0f / sqrtf(4.0f*M_PI);
	for (int n = 0; n<W.rows(); ++n) {
		W.row(n) = W.row(n) * sqrtf(2.f * n + 1) * factor;
	}
...
template<class Vector>
inline Eigen::MatrixXf ZonalWeightsEigen(const std::vector<Vector>& directions) {
	...
	const Eigen::MatrixXf ZW = ZonalWeightsEigen(order);
...
inline Eigen::VectorXf ZonalNormalizationEigen(int order) {
	Eigen::VectorXf res = Eigen::VectorXf(order);
	const float f = 1.0f / sqrt(4.0f*M_PI);
	for (int i = 0; i<order; ++i) {
		res[i] = sqrtf(2.f * i + 1)*f;
	}
	return res;
}

template<class Vector>
inline Eigen::MatrixXf ZonalExpansionEigen(const std::vector<Vector>& directions) {
	...
	const auto zhNorm = ZonalNormalizationEigen(order);
	...
	for (int i = 0; i<dsize; ++i) {
		...
		const Vector w(directions[i].x, directions[i].z, directions[i].y);

		// Evaluate all the Y_{l,m} for the current vector
		const int lmax = order - 1;
		const auto size = SHTerms(lmax);
		Eigen::VectorXf ylm(size);
		SHEvalFast<Vector>(w, lmax, ylm);
...
inline Eigen::MatrixXf computeInverseEigen(const Eigen::MatrixXf& Y) {
	const int nrows = int(Y.rows());
	const int order = int(sqrt(nrows));

#ifdef USE_SPARSE_EIGEN
	MatrixType A(mrows, nrows);
	std::vector<Eigen::Triplet<float>> triplets;
#else
	Eigen::MatrixXf A = Eigen::MatrixXf::Zero(nrows, nrows);
#endif
...
inline int TestZHValidation(u32 n) {
	const f32 eps = 1e-2f;

	LogInfo("Zonal Weights for order = ", n);
	f32 *weights = new f32[n*n]();

	f64 t1 = glfwGetTime();
	ZonalWeights(n, weights);
	t1 = glfwGetTime() - t1;
	LogInfo("Mine (", t1, ") : ");

	//for (u32 i = 0; i < n; ++i) {
	//	for (u32 j = 0; j < n; ++j) {
	//		std::cout << weights[i*n + j] << " ";
	//	}
	//	std::cout << std::endl;
	//}

	t1 = glfwGetTime();
	Eigen::MatrixXf weightsEigen = ZonalWeightsEigen(n);
	t1 = glfwGetTime() - t1;
	LogInfo("Eigen (", t1, ") : ");
	...
	for (u32 i = 0; i < n; ++i) {
		for (u32 j = 0; j < n; ++j) {
			f32 a = weightsEigen(i, j);
			f32 b = weights[i*n + j];
			if (std::fabsf(a - b) > eps) {
				//LogInfo("Err [", i, ",", j, "] : ", a, " vs ", b);
				error++;
			}
		}
	}
	totalError += error;
	LogInfo("Error count : ", error);

	// Test Direction Zonal Weights
	const u32 dirCount = 2 * n + 1;
	const u32 order = (dirCount - 1) / 2 + 1;
	const u32 mrows = order*order;
	const u32 mcols = dirCount*order;

	LogInfo("Direction Zonal Weights for dirs = ", dirCount);

	std::vector<vec3f> directions;
	//Sampling::SampleSphereRandom(directions, dirCount);
	Sampling::SampleSphereBluenoise(directions, dirCount);
	f32 *ZW = new f32[mcols*mrows]();

	t1 = glfwGetTime();
	ZonalWeights((f32*) &directions[0], dirCount, ZW);
	t1 = glfwGetTime() - t1;

	LogInfo("Mine (", t1, ") : ");

	t1 = glfwGetTime();
	weightsEigen = ZonalWeightsEigen(directions);
	t1 = glfwGetTime() - t1;

	LogInfo("Eigen (", t1, ") : ");

	error = 0;
	for (u32 i = 0; i < mrows; ++i) {
		for (u32 j = 0; j < mcols; ++j) {
			f32 a = weightsEigen(i, j);
			f32 b = ZW[i*mcols + j];
			if (std::fabsf(a - b) > eps) {
				//LogInfo("Err [", i, ",", j, "] : ", a, " vs ", b);
	...
	LogInfo("Zonal Expansion for dirs = ", dirCount);

	f32 *ZHE = new f32[mrows*mrows]();

	t1 = glfwGetTime();
	ZonalExpansion((f32*) &directions[0], dirCount, ZHE);
	t1 = glfwGetTime() - t1;

	LogInfo("Mine (", t1, ") : ");

	t1 = glfwGetTime();
	weightsEigen = ZonalExpansionEigen(directions);
	t1 = glfwGetTime() - t1;
	...
	LogInfo("Inverse Zonal Expansion for dirs = ", dirCount);
	{

		t1 = glfwGetTime();
		InverseZonalExpansion((f32*) &directions[0], dirCount, ZHE);
		t1 = glfwGetTime() - t1;
		...
		t1 = glfwGetTime();
		weightsEigen = ZonalExpansionEigen(directions);
		Eigen::MatrixXf A = computeInverseEigen(weightsEigen);
		t1 = glfwGetTime() - t1;
	...
	LogInfo("AP matrix for dirs = ", dirCount);
	{
		f32 *AP = new f32[mcols*mrows]();

		t1 = glfwGetTime();
		ComputeAPMatrix((f32*) &directions[0], dirCount, AP);
		t1 = glfwGetTime() - t1;

		LogInfo("Mine (", t1, ") : ");

		t1 = glfwGetTime();
		Eigen::MatrixXf A = computeInverseEigen(ZonalExpansionEigen(directions));
		Eigen::MatrixXf P = ZonalWeightsEigen(directions);
		Eigen::MatrixXf APEigen = A * P;
		t1 = glfwGetTime() - t1;
